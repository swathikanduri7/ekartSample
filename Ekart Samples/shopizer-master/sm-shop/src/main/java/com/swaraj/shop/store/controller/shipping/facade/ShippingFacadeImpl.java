...
import com.swaraj.core.business.services.shipping.ShippingOriginService;
...
import com.swaraj.core.model.shipping.ShippingPackageType;
import com.swaraj.core.model.shipping.ShippingType;
import com.swaraj.shop.model.references.PersistableAddress;
import com.swaraj.shop.model.references.ReadableAddress;
import com.swaraj.shop.model.references.ReadableCountry;
import com.swaraj.shop.model.shipping.ExpeditionConfiguration;
...
import com.swaraj.shop.store.api.exception.OperationNotAllowedException;
...
import com.swaraj.shop.store.controller.shipping.facade.ShippingFacade;
...
public class ShippingFacadeImpl implements ShippingFacade {
	...
	public void createPackage(PackageDetails packaging, MerchantStore store) {
		...
		com.swaraj.core.model.shipping.Package pack = toPackage(packaging);
	...
	private boolean packageExists(ShippingConfiguration configuration, PackageDetails packageDetails) {
		...
		List<com.swaraj.core.model.shipping.Package> packages = configuration.getPackages().stream().filter(p -> p.getCode().equalsIgnoreCase(packageDetails.getCode())).collect(Collectors.toList());
	...
	private com.swaraj.core.model.shipping.Package packageDetails(ShippingConfiguration configuration, String code) {
		
		Validate.notNull(configuration,"ShippingConfiguration cannot be null");
		Validate.notNull(code, "PackageDetails code cannot be null");

		List<com.swaraj.core.model.shipping.Package> packages = configuration.getPackages().stream().filter(p -> p.getCode().equalsIgnoreCase(code)).collect(Collectors.toList());
	...
	public PackageDetails getPackage(String code, MerchantStore store) {
		...
		com.swaraj.core.model.shipping.Package p = this.packageDetails(config, code);
	...
	public void updatePackage(String code, PackageDetails packaging, MerchantStore store) {
		...
		//need to check if code exists
		List<com.swaraj.core.model.shipping.Package> packs = config.getPackages().stream().filter(pa -> !pa.getCode().equals(code)).collect(Collectors.toList());
		packs.add(pack);
	...
	public void deletePackage(String code, MerchantStore store) {
		...
		List<com.swaraj.core.model.shipping.Package> packages = config.getPackages();
		
		List<com.swaraj.core.model.shipping.Package> packList = config.getPackages().stream().filter(p -> p.getCode().equalsIgnoreCase(code)).collect(Collectors.toList());
	...
	private PackageDetails toPackageDetails(com.swaraj.core.model.shipping.Package pack) {
	...
	private com.swaraj.core.model.shipping.Package toPackage(PackageDetails pack) {
		com.swaraj.core.model.shipping.Package details = new com.swaraj.core.model.shipping.Package();
		details.setCode(pack.getCode());
