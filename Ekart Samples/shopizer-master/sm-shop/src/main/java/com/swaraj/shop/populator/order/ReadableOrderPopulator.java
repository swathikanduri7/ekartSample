...
/*			ReadableMerchantStorePopulator merchantPopulator = new ReadableMerchantStorePopulator();
		...
		com.swaraj.shop.model.order.total.OrderTotal taxTotal = null;
		com.swaraj.shop.model.order.total.OrderTotal shippingTotal = null;
		...
		if(source.getOrderAttributes()!=null && source.getOrderAttributes().size()>0) {
			for(OrderAttribute attr : source.getOrderAttributes()) {
				com.swaraj.shop.model.order.OrderAttribute a = new com.swaraj.shop.model.order.OrderAttribute();
		...
		List<com.swaraj.shop.model.order.total.OrderTotal> totals = new ArrayList<com.swaraj.shop.model.order.total.OrderTotal>();
		for(OrderTotal t : source.getOrderTotal()) {
			...
			if(t.getOrderTotalType().name().equals(OrderTotalType.TOTAL.name())) {
				com.swaraj.shop.model.order.total.OrderTotal totalTotal = createTotal(t);
				...
				com.swaraj.shop.model.order.total.OrderTotal subTotal = createTotal(t);
				totals.add(subTotal);
				
			}
			else {
				com.swaraj.shop.model.order.total.OrderTotal otherTotal = createTotal(t);
	...
	private com.swaraj.shop.model.order.total.OrderTotal createTotal(OrderTotal t) {
		com.swaraj.shop.model.order.total.OrderTotal totalTotal = new com.swaraj.shop.model.order.total.OrderTotal();
