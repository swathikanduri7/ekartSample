 ...
 * 
 */
package com.swaraj.shop.populator.shoppingCart;
    ...
    extends AbstractDataPopulator<ShoppingCartData,ShoppingCart>
{
    ...
    public ShoppingCart populate(ShoppingCartData shoppingCart,ShoppingCart cartMdel,final MerchantStore store, Language language)
    {
       ...
       try{
        ...
        Set<com.swaraj.core.model.shoppingcart.ShoppingCartItem> newItems =
        ...
        if ( items != null && items.size() > 0 )
        {
            for ( ShoppingCartItem item : items )
            {
                ...
                Set<com.swaraj.core.model.shoppingcart.ShoppingCartItem> cartItems = cartMdel.getLineItems();
                if ( cartItems != null && cartItems.size() > 0 )
                {

                    for ( com.swaraj.core.model.shoppingcart.ShoppingCartItem dbItem : cartItems )
                    {
                        if ( dbItem.getId().longValue() == item.getId() )
                        {
                            dbItem.setQuantity( item.getQuantity() );
                            // compare attributes
                            Set<com.swaraj.core.model.shoppingcart.ShoppingCartAttributeItem> attributes =
                                dbItem.getAttributes();
                            Set<com.swaraj.core.model.shoppingcart.ShoppingCartAttributeItem> newAttributes =
                                new HashSet<com.swaraj.core.model.shoppingcart.ShoppingCartAttributeItem>();
                            List<ShoppingCartAttribute> cartAttributes = item.getShoppingCartAttributes();
                            if ( !CollectionUtils.isEmpty( cartAttributes ) )
                            {
                                for ( ShoppingCartAttribute attribute : cartAttributes )
                                {
                                    for ( com.swaraj.core.model.shoppingcart.ShoppingCartAttributeItem dbAttribute : attributes )
                                    {
                ...
                {// create new item
                    com.swaraj.core.model.shoppingcart.ShoppingCartItem cartItem =
                        createCartItem( cartMdel, item, store );
                    Set<com.swaraj.core.model.shoppingcart.ShoppingCartItem> lineItems =
                        cartMdel.getLineItems();
                    if ( lineItems == null )
                    {
                        lineItems = new HashSet<com.swaraj.core.model.shoppingcart.ShoppingCartItem>();
    ...
    private com.swaraj.core.model.shoppingcart.ShoppingCartItem createCartItem( com.swaraj.core.model.shoppingcart.ShoppingCart cart,
        ...
        throws Exception
    {
        ...
        com.swaraj.core.model.shoppingcart.ShoppingCartItem item =
            new com.swaraj.core.model.shoppingcart.ShoppingCartItem( cart, product );
