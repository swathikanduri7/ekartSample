...
import com.swaraj.shop.model.catalog.product.LightPersistableProduct;
...
import com.swaraj.shop.model.catalog.product.ProductPriceRequest;
...
import com.swaraj.shop.populator.catalog.PersistableProductPopulator;
import com.swaraj.shop.populator.catalog.PersistableProductReviewPopulator;
import com.swaraj.shop.populator.catalog.ReadableFinalPricePopulator;
import com.swaraj.shop.populator.catalog.ReadableProductPopulator;
import com.swaraj.shop.populator.catalog.ReadableProductReviewPopulator;
...
import com.swaraj.core.model.catalog.product.relationship.ProductRelationshipType;
	...
	public ReadableProductList getProductListsByCriterias(MerchantStore store, Language language,
		...
		if (CollectionUtils.isNotEmpty(criterias.getCategoryIds())) {
			...
			if (criterias.getCategoryIds().size() == 1) {
				...
				com.swaraj.core.model.catalog.category.Category category = categoryService
				...
				if (category != null) {
					...
					List<com.swaraj.core.model.catalog.category.Category> categories = categoryService
							.getListByLineage(store, lineage);

					List<Long> ids = new ArrayList<Long>();
					if (categories != null && categories.size() > 0) {
						for (com.swaraj.core.model.catalog.category.Category c : categories) {
							ids.add(c.getId());
						}
					}
		...
		//com.swaraj.core.model.catalog.product.ProductList products = productService.listByStore(store, language,
		//		criterias);
