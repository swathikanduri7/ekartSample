	...
	public Manufacturer populate(PersistableManufacturer source,
		...
		try {
			...
			if(!CollectionUtils.isEmpty(source.getDescriptions())) {
				Set<com.swaraj.core.model.catalog.product.manufacturer.ManufacturerDescription> descriptions = new HashSet<com.swaraj.core.model.catalog.product.manufacturer.ManufacturerDescription>();
				for(ManufacturerDescription description : source.getDescriptions()) {
					com.swaraj.core.model.catalog.product.manufacturer.ManufacturerDescription desc = new com.swaraj.core.model.catalog.product.manufacturer.ManufacturerDescription();
					if(desc.getId() != null && desc.getId().longValue()>0) {
						desc.setId(description.getId());
					}
					if(target.getDescriptions() != null) {
						for(com.swaraj.core.model.catalog.product.manufacturer.ManufacturerDescription d : target.getDescriptions()) {
